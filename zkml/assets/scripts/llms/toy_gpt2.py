...
def map_pytorch_to_gguf_name(pytorch_name):
    ...
    print(
        f"Warning: No mapping found for parameter '{pytorch_name}', using original name"
    )
    return pytorch_name
...
def export_to_gguf(model, config, filename="toy_gpt2.gguf"):
    ...
    ), f"Token count {len(tokens)} doesn't match config vocab size {config.vocab_size}"
    ...
    eos_token_id = (
        ...
        if gpt2_tokenizer.eos_token_id is not None
        else 50256
    )
    bos_token_id = (
        gpt2_tokenizer.bos_token_id
        if gpt2_tokenizer.bos_token_id is not None
        else eos_token_id
    )  # GPT-2 uses EOS as BOS
    unk_token_id = (
        gpt2_tokenizer.unk_token_id
        if gpt2_tokenizer.unk_token_id is not None
        else eos_token_id
    )
    pad_token_id = (
    ...
    if unk_token_id < len(token_types) and unk_token_id not in [
        eos_token_id,
        bos_token_id,
    ]:
    ...
    print(
        f"Special tokens - EOS: {eos_token_id}, BOS: {bos_token_id}, UNK: {unk_token_id}, PAD: {pad_token_id}"
    )
