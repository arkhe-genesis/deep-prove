...
parser = argparse.ArgumentParser(
    description="Run a GPT-2 model and dump weights and intermediate tensors."
)
parser.add_argument(
    "--output-dir",
    type=str,
...
with torch.no_grad():
    ...
    position_ids = torch.arange(
        0, input_seq_length, dtype=torch.long, device=input_ids.device
    ).unsqueeze(0)
    ...
    assert torch.allclose(
        inputs_embeds, token_embeds_only + pos_embeds_only, atol=1e-6
...
def to_list(t):
    return t.squeeze().flatten().tolist()
...
with torch.no_grad():
    ...
    for i, layer in enumerate(model.transformer.h):
        ...
        if i == 5:
            ...
            print(
                f"🔍 Layer {i} ln1_out vs reference: max diff = {ln1_diff.item():.8f}"
            )
            if ln1_diff > 1e-6:
                print(f"❌ Layer {i} ln1_out already diverged from reference!")
                print(f"  Manual ln1_out first 3: {ln1_out.flatten()[:3].tolist()}")
                print(
                    f"  Reference ln1_out first 3: {reference_ln1_out.flatten()[:3].tolist()}"
                )
        ...
        # a. QKV projection
        q_before_heads, k_before_heads, v_before_heads = layer.attn.c_attn(
            ln1_out
        ).split(model.config.hidden_size, dim=2)
        ...
        if not torch.allclose(comparison_manual_output, automated_output, atol=1e-5):
            ...
            if i == 5:
                ...
                print(
                    f"  Hidden states match: {torch.allclose(hidden_states, ref_hidden_states, atol=1e-6)}"
                )
                print(
                    f"  LN1 outputs match: {torch.allclose(ln1_out, ref_ln1_out, atol=1e-6)}"
                )
                ...
                print(
                    f"  Attention outputs match: {torch.allclose(our_attn_out, ref_attn_out, atol=1e-6)}"
                )

                # Check residual after attention
                ref_residual_attn = ref_hidden_states + ref_attn_out
                print(
                    f"  Residual attn match: {torch.allclose(residual_attn, ref_residual_attn, atol=1e-6)}"
                )
                ...
                print(
                    f"  LN2 outputs match: {torch.allclose(ln2_out, ref_ln2_out, atol=1e-6)}"
                )
                print(
                    f"  MLP outputs match: {torch.allclose(our_mlp_out, ref_mlp_out, atol=1e-6)}"
                )
                ...
                print(
                    f"  Final outputs match: {torch.allclose(our_final, ref_final, atol=1e-6)}"
                )
                print(
                    f"  Reference final (first 3): {ref_final.flatten()[:3].tolist()}"
                )
                print(f"  Our final (first 3): {our_final.flatten()[:3].tolist()}")
                print(
                    f"  Manual calc (first 3): {manual_output.flatten()[:3].tolist()}"
                )
                print(
                    f"  Automated (first 3): {automated_output.flatten()[:3].tolist()}"
                )

                # Sanity check: manual_output should equal our_final
                print(
                    f"  manual_output == our_final: {torch.allclose(manual_output, our_final, atol=1e-8)}"
                )

                # Double check the transformer reference calculation
                ref_block_output = layer(ref_hidden_states)[0]
                print(
                    f"  Direct block call matches automated: {torch.allclose(ref_block_output, automated_output, atol=1e-6)}"
                )

                # Test: What if we call the SAME layer on the SAME hidden state multiple times?
                ref_block_output2 = layer(ref_hidden_states)[0]
                print(
                    f"  Layer is deterministic: {torch.allclose(ref_block_output, ref_block_output2, atol=1e-8)}"
                )

                # The issue might be that transformer_outputs.hidden_states comes from a different execution context
                print(
                    f"  Block output diff from transformer: {(ref_block_output - automated_output).abs().max().item():.8f}"
                )
                print(
                    f"  Block vs transformer (first 3): block={ref_block_output.flatten()[:3].tolist()}"
                )
                print(
                    f"  Block vs transformer (first 3): transformer={automated_output.flatten()[:3].tolist()}"
                )
        else:
        ...
        if manual_output_with_final_ln is not None:
            layer_json["manual_output_with_final_ln"] = to_list(
...
output_json = {
    ...
    "final_output": (
        to_list(manual_output_with_final_ln)
        if manual_output_with_final_ln is not None
        else None
    ),
}
...
with torch.no_grad():
    ...
    print(
        f"  Logits match: {torch.allclose(manual_logits, official_logits, atol=1e-5)}"
    )
...
export = {"metadata": export_metadata, "tensors": {}}
...
for name, tensor in state_dict.items():
        ...
        if hf_subname in conv1d_module_suffixes:
            try:
                ...
                for part in module_path_parts:
                    current_module_for_type_check = getattr(
                        current_module_for_type_check, part
                    )
                assert isinstance(
                    current_module_for_type_check, Conv1D
                ...
                if hf_subname == "attn.c_attn.weight":
                    ...
                    print(
                        f"ℹ️ Exporting {name} : Original Conv1D shape {list(processed_tensor.shape)}. Exporting concatenated W_q.T, W_k.T, W_v.T data as 1D array, shape {current_shape}."
                    )
                elif hf_subname == "attn.c_proj.weight":
                    current_shape = list(processed_tensor.shape)
                    current_data_list = processed_tensor.numpy().flatten().tolist()
                    print(
                        f"ℹ️ Exporting {name}: Original Conv1D shape {list(processed_tensor.shape)}, NOT transposing."
                    )
                    print(f"\t\t{current_data_list}\n")
                elif hf_subname.endswith(
                    ".weight"
                ):  # Other Conv1D weights (e.g., mlp.c_fc, mlp.c_proj, attn.c_proj)
                    ...
                    print(
                        f"ℹ️ Exporting {name}: Original Conv1D shape {list(processed_tensor.shape)}, transposing to {current_shape} for export."
                    )
                    print(f"\t\t{current_data_list}\n")
                ...
                print(
                    f"⚠️ Warning: Could not access module for {name} to assert its type for Conv1D check."
                )
            ...
            try:
                ...
                print(
                    f"⚠️ Warning: Could not access module for {name} (subname: {hf_subname}) to assert its type properties (non-Conv1D check)."
                )
            except AssertionError as e:
                print(f"❌ Critical Error for {name}: {e}")
                raise e
    ...
    if all(x == 0.0 for x in current_data_list):
        print(
            f"⚠️ Warning: Tensor '{name}' (Exported Shape: {current_shape}) consists entirely of zeros."
        )

    tensor_data = {"shape": current_shape, "data": current_data_list}
    ...
    if name in hf_to_canonical_map:
        ...
        if match:
            ...
            print(
                f"⚠️ Warning: Tensor '{name}' (Shape: {tensor.shape}) not mapped to a canonical name and not part of a block. Ensure it's handled or explicitly ignored."
            )
            if "others" not in export:
                export["others"] = {}
...
if args.export_model:
    weights_fname = os.path.join(
        args.output_dir, f"{args.model_name.replace('-', '_')}_weights.json"
    )
