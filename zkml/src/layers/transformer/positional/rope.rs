...
impl<N: Number> Rope<N> {
        ...
        N: burn::tensor::Element,
    {
        ...
        positional_cache
            ...
            cosine_slice_bt.shape() == input_bt.shape(),
        ...
        let half = input.shape()[1] / 2;

        let inputs = input_bt.reshape([rows, half, 2]);
        let cosines = cosine_slice_bt.reshape([rows, half, 2]);
        let sines = sine_slice_bt.reshape([rows, half, 2]);
        ...
        let x1 = inputs
            ...
            .reshape([rows, half]);
        let x2 = inputs.slice([0..rows, 0..half, 1..2]).reshape([rows, half]);
        let c1 = cosines
            .clone()
            .slice([0..rows, 0..half, 0..1])
            .reshape([rows, half]);
        ...
        let s1 = sines
            ...
            .slice([0..rows, 0..half, 0..1])
            .reshape([rows, half]);
        let s2 = sines.slice([0..rows, 0..half, 1..2]).reshape([rows, half]);
        ...
        let out_even_e = out_even.reshape([rows, half, 1]);
        let out_odd_e = out_odd.reshape([rows, half, 1]);
        let output_bt = BTensor::cat(vec![out_even_e, out_odd_e], 2).reshape([rows, half * 2]);
        let out_vec: Vec<N> = output_bt
            .to_data()
            .into_vec()
            .expect("convert burn tensor to scalar data");
        let output = Tensor::<N>::new(vec![rows, half * 2].into(), out_vec);
...
mod tests {
    ...
    fn rope_input<T: Number>() -> impl Strategy<Value = Input<T>> {
    ...
    proptest! {
        ...
        fn test_rope_f32(inp in rope_input::<f32>()) {
            ...
            let out = is_close_with_tolerance(out.data(), expected_t.data(), 1e-5, 1e-4);
            prop_assert!(out);
        }
